...
OpenKinect::Tick()
{
    ...
    int ret = freenect_sync_get_depth((void**)(&depth_buf), &timestamp, 0, FREENECT_DEPTH_11BIT); // FREENECT_DEPTH_11BIT, FREENECT_DEPTH_REGISTERED, FREENECT_DEPTH_MM
    ...
    for(int j=0; j<480; j++)
        for(int i=0; i<640; i++)
            depth[j][i] = float(depth_buf[s++])/1023.0;
}
